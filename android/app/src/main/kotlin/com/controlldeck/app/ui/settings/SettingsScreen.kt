...
fun SettingsScreen(viewModel: SettingsViewModel) {
    ...
    Scaffold(topBar = { TopAppBar(title = { Text("Settings") }) }) { padding ->
        Column(modifier = Modifier.fillMaxSize().padding(padding).padding(16.dp)) {
            ...
            Text("Auto-accept pairing", style = MaterialTheme.typography.titleMedium, modifier = Modifier.padding(top = 24.dp))
            Text(
                "Testing only. When on, this device accepts any pairing request over the LAN with one click — no PIN or QR code needed. " +
                    "Anything on your network speaking the protocol can pair with it while this is enabled.",
                style = MaterialTheme.typography.bodySmall,
                color = MaterialTheme.colorScheme.error,
            )
            Row(verticalAlignment = Alignment.CenterVertically, modifier = Modifier.padding(top = 4.dp)) {
                Switch(checked = state.autoAcceptPairing, onCheckedChange = { viewModel.setAutoAcceptPairing(it) })
                Text(if (state.autoAcceptPairing) "Enabled" else "Disabled", modifier = Modifier.padding(start = 8.dp))
            }
